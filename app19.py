...
app = FastAPI()
LOG_DIRS = ["logs", "services/logs", "external/logs"]  # Support multiple directories
...
@app.get("/health")
def health_check():
    return {"status": "ok", "log_files": list(cached_logs.keys())}
...
@app.post("/search")
def search(query: str = Body(...), include_ai: bool = Body(False)):
    results = search_logs(cached_logs, query)
    ai_lines = []
    if include_ai:
        for filename, entries in cached_logs.items():
            for entry in entries:
                if rapidfuzz.fuzz.partial_ratio(query.lower(), entry.lower()) > 75:
                    ai_lines.append(entry)
