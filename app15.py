...
# ✅ Disable ONNX Custom Ops (if not using them)
os.environ["DISABLE_ONNX_CUSTOM_OPS"] = "1"
...
# ✅ Load State Dict Efficiently (Convert to float32 for CPU)
state_dict = torch.load(MODEL_FILE, map_location="cpu")
state_dict = {k: v.to(dtype=torch.float32) for k, v in state_dict.items()}  # ⚡ Optimized for CPU
...
async def search(query: str = Query(..., title="Search Keyword")):
    logs = await asyncio.to_thread(read_logs_from_directory, LOG_DIR)
    results = await asyncio.to_thread(search_logs, logs, query)
    ai_insights = await asyncio.to_thread(generate_ai_insights, results)
