...
# ✅ Load Open-LLaMA-13B Model Fully Locally
MODEL_PATH = "C:/path/to/Open-LLaMA-13B/consolidated.00.pth"
TOKENIZER_PATH = "C:/path/to/Open-LLaMA-13B/tokenizer.model"

print("Loading Open-LLaMA-13B model locally...")
tokenizer = LlamaTokenizer(TOKENIZER_PATH)
model = LlamaForCausalLM.from_pretrained(MODEL_PATH, state_dict=torch.load(MODEL_PATH, map_location="cpu"))
model.eval()
print("Model loaded successfully!")


def read_logs_from_directory(directory: str) -> Dict[str, List[str]]:
    """Reads logs from a directory including normal and gzipped logs."""
    logs = {}
    path = Path(directory)
...
def generate_ai_summary(log_entries: List[str]) -> str:
    """Uses Open-LLaMA-13B (locally) to summarize extracted logs."""
    if not log_entries:
